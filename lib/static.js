...
/**
 * Use the given plugin `fn()`. If `client` is specified, only use the plugin on the appropriate client.
 *
 * @param {String} [client]
 * @param {Function} fn
 * @return {Function} self
...
exports.use = function(client, fn){
  if(!fn) {
    fn = client;
    client = undefined;
  }
  ...
  if(!client) {
    ...
    if(isBrowser && rbrowser.test(client)) fn(this);
    else if(!isBrowser && rserver.test(client)) fn(this);
  }
  return this;
};
