...
exports.attr = function(name, options){
  ...
  this.prototype[name] = function(val){
    ...
    var changed = false;
    if(type(this.attrs[name]) == 'object') {
      for(var key in this.attrs[name]) {
        if(val[key] === undefined) {
          changed = true;
          break;
        } else {
          if(val[key] !== this.attrs[name][key]) {
            changed = true;
            break;
          }
        }
      }
      if(Object.keys(this.attrs[name]).length != Object.keys(val).length)
        changed = true;
    } else if (type(this.attrs[name]) == 'array') {
      if(this.attrs[name].length != val.length)
        changed = true;
      else
        for(var i = 0, setItem, newItem; setItem = this.attrs[name][i] && newItem = val[i]; ++i) {
          if(setItem != newItem) {
            changed = true;
            break;
          }
